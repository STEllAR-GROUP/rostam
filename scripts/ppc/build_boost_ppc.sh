...
$BJAM --stagedir=$DIRECTORY/debug variant=debug $BJAM_FLAGS -j${THREADS} 
if ! [[ $? == "0" ]]; then echo "ERROR: Debug build of Boost failed"; error; fi

$BJAM --stagedir=$DIRECTORY/release variant=release $BJAM_FLAGS -j${THREADS}
...
MODULE_FILE=$MODULE_DIRECTORY/boost/$MODULE_WORD-release
if [ -f $MODULE_FILE ]
   then rm $MODULE_FILE; fi

echo "#%Module" >> $MODULE_FILE
echo "# Generated by build_boost.sh" >> $MODULE_FILE
echo "proc ModulesHelp { } {" >> $MODULE_FILE
echo "    puts stderr { Boost provides free peer-reviewed portable C++ source libraries. - Homepage: http://www.boost.org/" >> $MODULE_FILE
echo "    }" >> $MODULE_FILE
echo "}" >> $MODULE_FILE
echo "" >> $MODULE_FILE
echo "module-whatis {Boost provides free peer-reviewed portable C++ source libraries. - Homepage: http://www.boost.org/}" >> $MODULE_FILE
echo "" >> $MODULE_FILE
echo "set root $DIRECTORY/release" >> $MODULE_FILE
echo "" >> $MODULE_FILE
echo "conflict boost" >> $MODULE_FILE
echo "" >> $MODULE_FILE
echo "prepend-path    CPATH           \$root" >> $MODULE_FILE
echo "prepend-path    LD_LIBRARY_PATH         \$root/lib" >> $MODULE_FILE
echo "prepend-path    LIBRARY_PATH            \$root/lib" >> $MODULE_FILE
echo "setenv  BOOST_ROOT            \"\$root\"" >> $MODULE_FILE
echo "setenv  BOOST_VERSION          \"$DOT_VERSION\"" >> $MODULE_FILE

MODULE_FILE_DEBUG=$MODULE_DIRECTORY/boost/$MODULE_WORD-debug
if [ -f $MODULE_FILE_DEBUG ]
   then rm $MODULE_FILE_DEBUG ; fi

echo "#%Module" >> $MODULE_FILE_DEBUG
echo "# Generated by build_boost.sh" >> $MODULE_FILE_DEBUG
echo "proc ModulesHelp { } {" >> $MODULE_FILE_DEBUG
echo "    puts stderr { Boost provides free peer-reviewed portable C++ source libraries. - Homepage: http://www.boost.org/" >> $MODULE_FILE_DEBUG
echo "    }" >> $MODULE_FILE_DEBUG
echo "}" >> $MODULE_FILE_DEBUG
echo "" >> $MODULE_FILE_DEBUG
echo "module-whatis {Boost provides free peer-reviewed portable C++ source libraries. - Homepage: http://www.boost.org/}" >> $MODULE_FILE_DEBUG
echo "" >> $MODULE_FILE_DEBUG
echo "set root $DIRECTORY/debug " >> $MODULE_FILE_DEBUG
echo "" >> $MODULE_FILE_DEBUG
echo "conflict boost" >> $MODULE_FILE_DEBUG
echo "" >> $MODULE_FILE_DEBUG
echo "prepend-path    CPATH           \$root" >> $MODULE_FILE_DEBUG
echo "prepend-path    LD_LIBRARY_PATH         \$root/lib" >> $MODULE_FILE_DEBUG
echo "prepend-path    LIBRARY_PATH            \$root/lib" >> $MODULE_FILE_DEBUG
echo "setenv  BOOST_ROOT            \"\$root\"" >> $MODULE_FILE_DEBUG
echo "setenv  BOOST_VERSION          \"$DOT_VERSION\"" >> $MODULE_FILE_DEBUG